...
const News = require('./src/books/news.model'); // Make sure this exists!
...
async function importData() {
    try {
        ...
        // Clear existing books and news
        await Book.deleteMany({});
        await News.deleteMany({});
        console.log("🗑️ Cleared existing books and news");

        // Add sample books
        const bookResult = await Book.insertMany(sampleBooks);
        console.log(`🎉 Successfully imported ${bookResult.length} books!`);

        // Add news
        const newsResult = await News.insertMany(news);
        console.log(`📰 Successfully imported ${newsResult.length} news items!`);
